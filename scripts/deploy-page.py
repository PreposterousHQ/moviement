...
Usage
  ...
  --extra SRC=TARGET additional file in the same deploy; repeatable. Widens
                     the payload guard to permit exactly these paths too.
...
# Netlify consumes these as configuration and never serves them back over HTTP,
# so they can never be fetched from live and must be seeded from the repo:
#   /netlify.toml  rewritten on every deploy (157 b in, 800+ b out)
#   /_redirects    parsed into routing rules; GET /_redirects returns 404
# Anything listed here MUST exist in the repo, or a later deploy cannot rebuild
# the payload and will abort rather than silently drop the file.
UNSERVABLE = {"/netlify.toml", "/_redirects"}
...
def build_payload(workdir, manifest):
    ...
    # Netlify lists /netlify.toml in the manifest but NOT /_redirects, which it
    # consumes into routing rules. An unservable file that never appears in the
    # manifest would therefore be absent from every future payload, and a
    # full-snapshot deploy would silently delete it. Seed them all from the repo.
    for p in sorted(UNSERVABLE):
        if p in seeded:
            continue
        src = os.path.join(REPO, p.lstrip("/"))
        if not os.path.isfile(src):
        ...
        seeded.append(p)
    for p in seeded:
        print("  %s seeded from repo (config, never fetched from live)" % p)
...
def main():
    ...
    ap.add_argument("--extra", action="append", default=[], metavar="SRC=TARGET")
    ...
    # Extra files ride along in the same deploy. They get no HTML prechecks --
    # they are not HTML -- but they widen the payload guard, so each one still
    # has to be named explicitly on the command line.
    extras = []
    for spec in a.extra:
        if "=" not in spec:
            die("--extra needs SRC=TARGET, got %r" % spec, 2)
        src, tgt = spec.split("=", 1)
        if not os.path.isfile(src):
            die("--extra source not found: " + src, 2)
        ed = open(src, "rb").read()
        if not ed:
            die("--extra source is empty: " + src, 2)
        et = "/" + tgt.strip("/")
        extras.append((src, et, ed))
        print("extra  : %s -> %s" % (src, et))
        print("         %d bytes   sha1 %s" % (len(ed), sha1(ed)))
    ...
    for _src, et, _ed in extras:
        if et not in before and not a.allow_new:
            die("%s is not currently live. Pass --allow-new to create it." % et)

    print("\nrebuilding payload from live bytes (%d files)" % len(manifest))
    workdir = tempfile.mkdtemp(prefix="moviement-deploy-")
    try:
        payload, seeded = build_payload(workdir, manifest)
        ...
        for _src, et, ed in extras:
            edest = os.path.join(payload, et.lstrip("/").replace("/", os.sep))
            os.makedirs(os.path.dirname(edest), exist_ok=True)
            open(edest, "wb").write(ed)
            if open(edest, "rb").read() != ed:
                die("payload copy of %s does not match source; refusing" % et)
            print("  swapped in %s (%d bytes, contents unmodified)" % (et, len(ed)))

        changed = diff_payload(payload, manifest)
        print("\npayload vs live:")
        for kind, rel in changed:
            print("  %-8s %s" % (kind, rel))
        expected = {target} | {e for _s, e, _d in extras} | set(seeded)
        ...
        for _src, et, ed in extras:
            if et in UNSERVABLE:
                # Netlify eats these as config, so GET must 404. A 200 would
                # mean the rule file is being served as page content.
                try:
                    st, _b = fetch(BASE_URL + et)
                except Exception as e:
                    st = getattr(e, "code", "err")
                print("  %-34s HTTP %s  (config, not served: expected)" % (et, st))
                if st == 200:
                    die("%s is served as content, which must never happen. "
                        "Roll back to %s." % (et, rollback))
                continue
            st, d = fetch(BASE_URL + page_url(et))
            print("  %-34s HTTP %s  %d bytes  %s"
                  % (et, st, len(d),
                     "byte-identical to source" if d == ed else "DIFFERS"))
            if d != ed:
                die("live bytes for %s do not match its source. Roll back to %s."
                    % (et, rollback))
        ...
        # An unservable file cannot be checked over HTTP, so its manifest sha1
        # is the only proof it landed with the bytes we sent.
        for _src, et, ed in extras:
            if et in UNSERVABLE:
                # Netlify does not list these in listSiteFiles at all, so there
                # is no manifest entry to check. Their effect is verified
                # functionally instead (e.g. the alias path returning 200).
                print("  manifest: %s not listed (config; verify by behaviour)" % et)
                continue
            if after.get(et) != sha1(ed):
                die("manifest sha1 for %s is %s, expected %s. Roll back to %s."
                    % (et, after.get(et), sha1(ed), rollback))
            print("  manifest: %s present with matching sha1" % et)

        if a.commit:
            shipped = [(a.source, target, len(data))]
            shipped += [(s, e, len(d)) for s, e, d in extras]
            for _s, _t, _n in shipped:
                repo_target = os.path.join(REPO, _t.lstrip("/").replace("/", os.sep))
                os.makedirs(os.path.dirname(repo_target), exist_ok=True)
                shutil.copyfile(_s, repo_target)
                subprocess.run(["git", "-C", REPO, "add", "--", repo_target], check=True)
            head_line = "Deploy " + ", ".join(_t for _s, _t, _n in shipped)
            lines = ["  %s (%d bytes)" % (_t, _n) for _s, _t, _n in shipped]
            msg = (head_line + chr(10) + chr(10)
                   + chr(10).join(lines) + chr(10) + chr(10)
                   + "Additive deploy from live bytes; CDN requested %d file(s). "
                     "Siblings verified byte-identical." % n
                   + chr(10) + "Rollback deploy id %s." % rollback + chr(10))
        ...
        for _src, et, ed in extras:
            print("ALSO       %s (%d bytes)" % (et, len(ed)))
